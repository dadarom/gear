...
object Master extends AkkaApp with ArgumentsParser {
  ...
  def master(ip:String, port : Int, akkaConf: Config): Unit = {
    ...
    val system = ActorSystem(MASTER, masterConfig) //akka://master

    val replicator = DataReplication(system).replicator // akka://master/user/replicator
    LOG.info(s"Replicator path: ${replicator.path}")

    //start master proxy              akka://master/user/master
    val masterProxy = system.actorOf(ClusterSingletonProxy.props(
      singletonPath = s"/user/${SINGLETON_MANAGER}/${MASTER_WATCHER}/${MASTER}",
      role = Some(MASTER)),
      name = MASTER)

    //start singleton manager akka://master/user/singleton
...
/**
 * master cluster -> cluster event listener
...
class MasterWatcher(role: String, masterProxy : ActorRef) extends Actor  with ActorLogging {
  ...
  override def preStart(): Unit = {
    cluster.subscribe(self, classOf[MemberEvent]) //register the subscriber to the specified Classifier
  ...
  def waitForInit : Receive = {
    case state: CurrentClusterState => { // Current snapshot state of the cluster
