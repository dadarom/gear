...
object AppMasterRuntimeEnvironment {
  ...
  def props(masters: Iterable[ActorPath], app : AppDescription, appContextInput: AppMasterContext): Props = {
    ...
    val masterFactory = (appId: AppId, masterProxy: MasterActorRef) =>
      MasterWithExecutorSystemProvider.props(appId, masterProxy)

    val appMasterFactory = (appContext: AppMasterContext, appDesc: AppDescription) =>
      LazyStartAppMaster.props(appContext, appDesc)
   ...
   *
   * master + executorSystemProvider [ExecutorSystemScheduler (executorSystemLauncher)]
   */
  private[appmaster]
