...
class ClusterConfig private(systemProperties : Config, custom : Config,
  ...
  def master : Config = {//merged reference.conf master config with gear.conf
...
object ClusterConfig {
  ...
  def load(source: ClusterConfigSource) : ClusterConfig = {
    ...
    /** http://doc.akka.io/docs/akka/current/general/configuration.html
      * load the normal config stack
      * (system props, then application.conf,then reference.conf)
      * */
